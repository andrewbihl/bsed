...
class TokenTree:
    ...
    def __init__(self, command_tree_spec: dict, translations: dict):
        self.command_tree = command_tree_spec
    ...
    def build_node_from_dict(self, node: dict, node_text: str, start_depth=0):
        ...
        if Keyword.REUSABLE_COMPONENTS.value in node:
            reusable_components = node.pop(Keyword.REUSABLE_COMPONENTS.value)
            for identifier in reusable_components:
                # Gets the expanded form of the referenced component
                reusable_component_dict = self.command_tree[Keyword.REUSABLE_COMPONENTS.value][identifier].copy()
                ...
                if len(after_reusable_component) != 0:
                    TokenTree._update_leaves_of_dict(reusable_component_dict, after_reusable_component)
